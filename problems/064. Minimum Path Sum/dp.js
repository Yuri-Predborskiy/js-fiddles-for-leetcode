/*
DP:
Add path from [0,0] to each point, save save sum as that point's value
Save smaller of two possible variants.
Time complexity: O(n) (every node visited only once)
Space complexity: O(1) because we directly modify inputs. O(n) if we don't modify inputs (for a copy of inputs)
 */
