...
Original optimized solution bit manipulations
Take current row of numbers (starting with 0)
Calculate 2 to the power of current index (1..n) (shift 1 by index bits to the left)
Add resulting number to every calculated number in reverse order
Amount of calculated numbers so far at each level is exactly the same as power of 2
This means result size is no longer required in calculations
Which means we can pre-define array size from the start and avoid costly array size recalculation in JS
...
let grayCode = function(n) {
    const results = new Array(2**n);
    results[0] = 0;
    for (let i = 0; i < n; i++) {
        const pow = 1 << i;
        for (let j = 0; j < pow; j++) {
            results[pow + j] = results[pow - j - 1] + pow;
        }
    }
    return results;
};
