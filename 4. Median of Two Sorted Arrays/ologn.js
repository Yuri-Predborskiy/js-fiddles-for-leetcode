...
function kth(arr1, arr2, start1, end1, start2, end2, k) {
    ...
    console.log('try', ++number, 'start1', start1, 'end1', end1, 'start2', start2, 'end2', end2, 'k', k);

    if (start1 === end1) {
        console.log('reached end of array 1, element not found, so it should be at index start2 + k in arr2');
        console.log(`looking at arr2[${start2 + k}], value is ${arr2[start2 + k]}`);
        return arr2[start2 + k];
    }
    if (start2 === end2) {
        console.log('reached end of array 2, element not found, so it should be at index start1 + k in arr1');
        console.log(`looking at arr1[${start1 + k}], value is ${arr1[start1 + k]}`);
        return arr1[start1 + k];
    }

    if (mid1 + mid2 < k) {
        // console.log(`mid1 (${mid1}) + mid2 (${mid2}) < k (${k})`);
        console.log(`mids (${mid1} + ${mid2} = ${mid1 + mid2}) < k (${k}), lets increase start and limit k`);
        if (arr1[start1 + mid1] < arr2[start2 + mid2]) {
            // console.log(`arr1[start1 + mid1] ${arr1[start1 + mid1]} < arr2[start2 + mid2] ${arr2[start2 + mid2]}`);
            console.log('limiting start of left and limiting k - mid1 - 1');
            return kth(arr1, arr2, start1 + mid1 + 1, end1, start2, end2, k - mid1 - 1);
        } else {
            // console.log(`arr1[start1 + mid1] ${arr1[start1 + mid1]} >= arr2[start2 + mid2] ${arr2[start2 + mid2]}`);
            console.log('limiting start of right and limiting k - mid2 - 1');
            return kth(arr1, arr2, start1, end1, start2 + mid2 + 1, end2, k - mid2 - 1);
        }
    } else {
        // console.log(`mid1 (${mid1}) + mid2 (${mid2}) >= k (${k})`);
        console.log(`mids (${mid1} + ${mid2} = ${mid1 + mid2}) >= k ${k}, lets limit end`);
        if (arr1[start1 + mid1] < arr2[start2 + mid2]) {
            // console.log(`arr1[start1 + mid1] ${arr1[start1 + mid1]} < arr2[start2 + mid2] ${arr2[start2 + mid2]}`);
            console.log('limiting end of right');
            return kth(arr1, arr2, start1, end1, start2, start2 + mid2, k);
        } else {
            // console.log(`arr1[start1 + mid1] ${arr1[start1 + mid1]} >= arr2[start2 + mid2] ${arr2[start2 + mid2]}`);
            console.log('limiting end of left');
